...
if (!function_exists('kleo_get_avatar')) {
    function kleo_get_avatar($image_args = array())
    {
        if (function_exists('bp_is_active')) {
...
function kleo_search_form($atts = array(), $content = null)
{
    ...
    $output .= '<input id="' . $input_id . '" class="' . $input_class . ' ajax_s" autocomplete="off" required type="text" name="' . $input_name . '" value="" placeholder="' . $input_placeholder . '">';
 ...
 */
function sq_modify_search_query( WP_Query $query ) {
	if ( is_search() && $query->is_main_query() && isset( $_GET['post_type'] ) ) { // Run only on the homepage
		$query->set( 'post_type', $_GET['post_type'] );
	}
}
add_action( 'pre_get_posts', 'sq_modify_search_query' );
...
/* Wisechat */
add_filter( 'body_class', 'sq_wise_chat_class' );
function sq_wise_chat_class( $class = array() ) {

	if ( class_exists( 'WiseChatContainer' ) && sq_option( 'wise_css', 1 ) == 1) {
		$class[] = 'buddyapp-wise-chat-pro';
	}
	return $class;
}
add_filter( 'kleo_theme_settings', 'kleo_wise_settings', 140 );

function kleo_wise_settings( $kleo ) {
	//
	// Settings Sections
	//
...
/**
 * Return Year for current-year shortcode
 * @param $atts
 * @param null $content
 *
 * @return false|string
 */
function sq_current_year_func( $atts, $content = null ) {
	return date('Y');
}
add_shortcode('current-year', 'sq_current_year_func');